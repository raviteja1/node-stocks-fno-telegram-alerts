...
Provider priority is fixed in the source code and cannot be changed with an environment variable:
...
- Recommended: an Upstox read-only Analytics Token.
...
```env
# Required for normal production use. Leave blank only when testing NSE fallback.
UPSTOX_ACCESS_TOKEN=
...
The service resumes a saved state only when `officialSnapshot=true` and `snapshotRequestedAt` maps to exactly 09:30:01 IST. Older, late, or test state is ignored automatically.
...
Test the complete provider flow without Telegram:

```bash
DRY_RUN=true ONE_POLL=true npm run scan-now
```

`scan-now` is only an immediate test. Its Telegram table is labeled `TEST SNAPSHOT`, it is not saved as the official daily state, and its current high/low values must not be used for production decisions.

Any breakout generated by this test mode is also labeled with its actual capture time:

```text
🧪 TEST POSITIVE BREAKOUT
Captured High (14:42:42 TEST): ₹385.40
```

A test alert is never labeled as a 09:30:01 breakout.

Send a test snapshot to Telegram and exit after one quote poll:

```bash
DRY_RUN=false ONE_POLL=true npm run scan-now
```

### Resume the official watchlist until 3:30 PM
...
Keep the terminal, computer, and internet connection running. If executed before 09:30:01, it waits and creates the official snapshot at 09:30:01. If executed later, it resumes only an already saved official 09:30:01 watchlist. If that official state does not exist, it refuses to create incorrect late reference levels.

### Run a temporary test

To start a clearly labeled, non-persistent test immediately:

```bash
npm run scan-now
```

Press `Ctrl+C` whenever you want to stop early. This mode also stops automatically at 15:30, but its high/low references are test values from the command start time—not official 09:30:01 values.
...
The process must already be running before 09:30:01 IST. At exactly 09:30:01, each selected gainer's current day high and each selected loser's current day low are saved as fixed references and are not updated for the rest of the day. If the process restarts later during market hours, it resumes only that validated official watchlist.
...
Google Cloud's eligible US-region `e2-micro` VM is an alternative if Oracle has no capacity, but it requires a billing account. Official Upstox is the primary source; the unofficial NSE fallback may be more likely to block a US cloud IP.
...
Ongoing maintenance is small but not zero: the Upstox Analytics Token eventually expires, unofficial NSE fallback endpoints may change, and free hosting policies can change. The morning Telegram table is the simplest daily confirmation that the service is healthy.
